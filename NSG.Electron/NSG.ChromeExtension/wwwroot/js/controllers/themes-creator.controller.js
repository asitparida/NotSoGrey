...
.controller('ThemesCreatorController', ['$state', 'SharedService', '$timeout', function ($state, SharedService, $timeout) {
    ...
    self.shared.loadActions(_actions, 'ThemesCreator');
    ...
    self.writeSCSSFile = function () {
        try {
            ...
            electron.ipcRenderer.on('write-scss-reply', function(event, arg){
                self.sharePanelShow = false;
    ...
    self.writePNGFile = function () {
        try {
            ...
            electron.ipcRenderer.on('write-png-reply', function(event, arg){
                self.sharePanelShow = false;
                self.shared.notifySave(arg);
            });
        } catch (e) {
    ...
    self.writeSVGFile = function () {
        try {
            ...
            electron.ipcRenderer.on('write-svg-reply', function(event, arg){
                self.sharePanelShow = false;
                self.shared.notifySave(arg);
            });
        } catch (e) {
    ...
    self.writeToPDF = function () {
        try {
            ...
            electron.ipcRenderer.on('write-pdf-reply', function(event, arg){
                self.sharePanelShow = false;
                self.shared.notifySave(arg);
            });
        } catch (e) {
    ...
    self.writeToASE = function () {
        try {
            ...
            electron.ipcRenderer.on('write-ase-reply', function(event, arg){
                self.sharePanelShow = false;
                self.shared.notifySave(arg);
            });
        } catch (e) {
