...
.service("SharedService", ["$http", "$q", "$window", "$state", "$resource", function ($http, $q, $window, $state, $resource) {
                ...
                chrome.storage.onChanged.addListener(function (changes, areaName) {
                    if (areaName == 'local' && changes['NSG_COLOR_DATA'] != null && changes['NSG_COLOR_DATA'] != {} && changes['NSG_COLOR_DATA']) {
                        if (typeof self.activeColorChanged !== 'undefined' && typeof self.activeColorChanged === 'function') {
                            self.activeColorChanged(tinycolor(changes['NSG_COLOR_DATA'].newValue).toHexString());
                        }
                    }
                });
            }
    ...
    console.log('hello ' + self.activeColor);
    self.actions = [];
    ...
    self.getDribbleShots = function (color) {
        ...
        try {
            if (self.electron != null) {
                ...
                self.electron.ipcRenderer.on('get-dribbble-reply', function (event, arg) {
                    _defer.resolve(arg);
                });
            }
            else
    ...
    self.getColorName = function (color) {
        ...
        try {
            if (self.electron != null) {
                ...
                self.electron.ipcRenderer.on('get-hexcodename-reply', function (event, arg) {
    ...
    //        animation: true,
    //        templateUrl: 'templates/nsg/palette-save-modal.html',
    //        backdrop: false,
