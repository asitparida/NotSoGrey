...
angular.module("NotSoGrey").controller("ColorPickerController",["$state","SharedService","$window","$timeout","$scope",function(e,t,n,a,o){var r=this;r.state=e,r.scope=o,r.shared=t,r.window=n,r.showElements=!1,r.animTop=!1,r.showBadge=!1;var i=[{id:2,name:"accept",icon:"icon-png accept",fn:"app.goColorDetails()",title:"Choose Color Shade",active:!0},{id:3,name:"copy",icon:"icon-png copy white",fn:"app.shared.copyHexToClipboard()",title:"Copy Hex Code",active:!0}];r.shared.loadActions(i,"ColorPicker"),r.shared.darkTheme=!0,r.topnav={isActive1:!0,isActive2:!1,isActive3:!1},r.name="Diablo Red",console.log(r.shared),r.color=r.origColorPrimary=r.shared.activeColor,console.log(r.origColorPrimary),console.log(r.shared.activeColor),r.foreColor="#fff",r.cmyk={c:80,m:33,y:20,k:14},r.rgb={r:80,g:10,b:2},r.hsb={h:80,s:10,b:2},r.hex="1ca32d";var c=!1,l=!1,s=!1,d=null,f=null,v=null;r.height=0,r.goDetails=function(){r.state.go("ColorDetails")},r.activateShade=function(e){_.each(r.shades,function(t){t.id==e.id?t.active=!t.active:t.active=!1,1==t.active&&(r.shared.activeColor=t.hexCode)})},r.canvas={id:_.uniqueId("canvasPicker"),top:null,instance:null,ctx:null,instanceBefore:null,instanceAfter:null,ctxBefore:null,ctxAfter:null,leftBefore:-340,leftAfter:340,colorCurrent:"#fff",colorAfter:"#fff",colorBefore:"#fff"},r.canvasIdBefore=_.uniqueId("canvasPicker"),r.canvasIdCurrent=_.uniqueId("canvasPicker"),r.canvasIdAfter=_.uniqueId("canvasPicker"),r.init=function(e){r.origColorPrimary=e;var t=r.window.innerHeight,n=r.window.innerWidth;r.canvas.instance=document.getElementById(r.canvasIdCurrent),r.canvas.instanceBefore=document.getElementById(r.canvasIdBefore),r.canvas.leftBefore=-window.innerWidth,r.canvas.leftAfter=window.innerWidth,r.canvas.instanceAfter=document.getElementById(r.canvasIdAfter),r.canvas.instance.height=r.canvas.instanceBefore.height=r.canvas.instanceAfter.height=t-48,r.canvas.instance.width=r.canvas.instanceBefore.width=r.canvas.instanceAfter.width=n,r.canvas.ctx=r.canvas.instance.getContext("2d"),r.canvas.ctxBefore=r.canvas.instanceBefore.getContext("2d"),r.canvas.ctxAfter=r.canvas.instanceAfter.getContext("2d");var o=r.canvas.ctx.createLinearGradient(0,0,0,r.canvas.instance.height);o.addColorStop(0,"white"),r.canvas.colorCurrent=e,o.addColorStop(.5,tinycolor(r.canvas.colorCurrent).toRgbString()),o.addColorStop(1,"black"),r.canvas.ctx.fillStyle=o,r.canvas.ctx.fillRect(0,0,n,r.canvas.instance.height),o=r.canvas.ctxBefore.createLinearGradient(0,0,0,r.canvas.instanceBefore.height),o.addColorStop(0,"white"),r.canvas.colorBefore=r.shared.getPreviousHueHex(e),o.addColorStop(.5,tinycolor(r.canvas.colorBefore).toRgbString()),o.addColorStop(1,"black"),r.canvas.ctxBefore.fillStyle=o,r.canvas.ctxBefore.fillRect(0,0,n,r.canvas.instanceBefore.height),o=r.canvas.ctxAfter.createLinearGradient(0,0,0,r.canvas.instanceAfter.height),o.addColorStop(0,"white"),r.canvas.colorAfter=r.shared.getNextHueHex(e),o.addColorStop(.5,tinycolor(r.canvas.colorAfter).toRgbString()),o.addColorStop(1,"black"),r.canvas.ctxAfter.fillStyle=o,r.canvas.ctxAfter.fillRect(0,0,n,r.canvas.instanceAfter.height),r.canvas.top=r.canvas.instance.height/2,document.getElementById("id_cardPannableContent").style.top=r.canvas.top+"px",r.currenTopPosition=r.canvas.top,r.getBadgeColor(),document.getElementById("id_cardPannableContent").focus(),0==r.animTop&&a(function(){r.animTop=!0,0==r.showBadge&&(r.showBadge=!0)},200)},angular.element(r.window).bind("resize",function(){r.init(r.shared.activeColor),r.scope.$$phase||r.scope.$apply()}),r.shared.initPicker=r.init,r.getBadgeColor=function(){var e=document.getElementById(r.canvasIdCurrent).getContext("2d").getImageData(r.canvas.instance.width/2,r.currenTopPosition,1,1).data,t={r:e[0],g:e[1],b:e[2]};r.badgeColor=tinycolor(t).toHexString(),r.badgeForeColor=r.shared.getForegrundContrastedColor(r.badgeColor),r.badgeShadow="0px 0px 26px 0px"+tinycolor(r.badgeForeColor).setAlpha(.05).toHexString(),r.shared.activeColor=r.color=r.badgeColor},a(function(){r.showElements=!0,chrome&&chrome.storage.local.get("NSG_COLOR_DATA",function(e){r.shared.activeColor=r.color=tinycolor(e.NSG_COLOR_DATA).toHexString(),a(r.init(r.color))})},33),r.pan=function(e,t){console.log("panstart "+t,e)},r.resetColor=function(){r.canvas.top=r.canvas.instance.height/2,document.getElementById("id_cardPannableContent").style.top=r.canvas.top+"px",r.currenTopPosition=r.canvas.top,r.getBadgeColor()},r.initializeCardPanElement=function(){0==s&&(d=document.getElementById("id_cardPannableContent"))},r.initializeCardPanElementBefore=function(){0==l&&(f=document.getElementById(r.canvasIdBefore))},r.initializeCardPanElementAfter=function(){0==c&&(v=document.getElementById(r.canvasIdAfter))},r.panStart=function(e){r.initializeCardPanElement(),r.initializeCardPanElementBefore(),r.initializeCardPanElementAfter(),r.topBeforePan=parseInt(d.style.top)||0,r.leftBeforePanForBefore=parseInt(f.style.left)||0,r.leftBeforePanForAfter=parseInt(v.style.left)||0},r.panvert=function(e,t){r.initializeCardPanElement();var n=(d.getBoundingClientRect(),document.getElementById("id_cardPannableContentHolder").getBoundingClientRect()),a=r.topBeforePan+(parseInt(e.deltaY)||0);r.maxTop=n.height-40,a>r.maxTop?a=r.maxTop:a<40&&(a=40),document.getElementById("id_cardPannableContent").style.top=a+"px",r.canvas.top=a,r.currenTopPosition=r.canvas.top,r.getBadgeColor()},r.panhorz=function(e,t){if("right"==t||"left"==t&&1==r.panRightStart){0==r.panRightStart&&(r.panRightStart=!0),r.initializeCardPanElementBefore();var n=r.leftBeforePanForBefore+(parseInt(e.deltaX)||0);r.canvas.leftBefore=n}else if("left"==t||"right"==t&&1==r.panLeftStart){0==r.panLeftStart&&(r.panLeftStart=!0),r.initializeCardPanElementAfter();var n=r.leftBeforePanForAfter+(parseInt(e.deltaX)||0);r.canvas.leftAfter=n}},r.panHorzEnd=function(e){var t=!1,n=!1;r.canvas.leftBefore>=-(.5*window.innerWidth)&&(r.init(r.canvas.colorBefore),t=!0),r.canvas.leftAfter<=.5*window.innerWidth&&(r.init(r.canvas.colorAfter),n=!0),t||(document.getElementById(r.canvasIdBefore).className+=" anim"),n||(document.getElementById(r.canvasIdAfter).className+=" anim"),r.canvas.leftBefore=-window.innerWidth,r.canvas.leftAfter=window.innerWidth,r.panLeftStart=!1,r.panRightStart=!1,a(function(){document.getElementById(r.canvasIdBefore).className=document.getElementById(r.canvasIdBefore).className.replace(" anim",""),document.getElementById(r.canvasIdAfter).className=document.getElementById(r.canvasIdAfter).className.replace(" anim","")},200)},r.mouseWheelUp=function(){r.initializeCardPanElement(),r.topBeforePan=parseInt(d.style.top)||0,r.panvert({deltaY:-5},"down")},r.mouseWheelDown=function(){r.initializeCardPanElement(),r.topBeforePan=parseInt(d.style.top)||0,r.panvert({deltaY:5},"up")},r.swipeLeft=function(){},r.swipeRight=function(){},r.keyUp=function(e){38==e.keyCode?r.mouseWheelUp():40==e.keyCode&&r.mouseWheelDown()}}]);
